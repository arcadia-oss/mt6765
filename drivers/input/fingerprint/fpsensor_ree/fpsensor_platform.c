...
//    fpsensor_spi_set_mode(fpsensor->spi, (u32)fpsensor->spi_freq_khz, 0);
	mutex_lock(&fpsensor->transfer_lock);	
    ...
    fpsensor_debug(DEBUG_LOG, "tony 0727 tx_len : %d \n", (int)len);
	...
	mutex_unlock(&fpsensor->transfer_lock);					   
#if FPSENSOR_MANUAL_CS
...
int fpsensor_check_HWID(fpsensor_data_t *fpsensor)
{
    ...
    hwid = ((rx[1] << 8) | (rx[2]));
    fpsensor_debug(ERR_LOG,"tony HWID 0x%x .\n",hwid);
    if ((hwid == 0x7153) || (hwid == 0x7230) ||(hwid == 0x7222)||(hwid == 0x7332)||(hwid == 0x7312)) {
        match = 1;
		//fingerprint_set_name("fpsensor_ree");
    }
    return match;
}

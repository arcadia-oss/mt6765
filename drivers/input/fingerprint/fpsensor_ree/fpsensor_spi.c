...
#if 1//def CONFIG_PM_WAKELOCKS
		fpsensor_timeout_wakelock=wakeup_source_register(NULL, "fpsensor timeout wakelock");
#else
...
static int fpsensor_probe(struct spi_device *spi)
{
 ...
 //   fpsensor_dev->spi_freq_khz = 8000u;
	...
	mutex_init(&fpsensor_dev->transfer_lock);
    // dts read
...
err2:
    mutex_destroy(&fpsensor_dev->transfer_lock);
    kfree(fpsensor_dev);
    FUNC_EXIT();
    return status;
}

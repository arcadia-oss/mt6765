...
#if defined(CONFIG_CTS_CHARGER_DETECT) || defined(CONFIG_CTS_EARPHONE_DETECT)
struct cts_device *cts_dev2;
bool tp_probe_done=false;
#endif

//start xjl 20191029
#define PROC_TP_VERSION "driver/tp_version"

char mtk_tp_version[128];
char mtk_tp_info[128];
...
static int proc_tp_version_open(struct inode *inode, struct file *file)
{
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    printk("  %-16s: %04x", "Firmware version", cts_dev->fwdata.version);
    ...
    printk("  %-16s: %02x", "Ddi verion", cts_dev->fwdata.ddi_version);

	//xjl 20191029
    sprintf((void*)mtk_tp_version,"0x%04x", cts_dev->fwdata.version);
	sprintf(mtk_tp_info, "%s", "chipone-tddi");
...
#ifdef CONFIG_CTS_EARPHONE_DETECT
    if (cts_is_earphone_exist(cts_dev)) {
        cts_earphone_plugin(cts_dev);
    }
#endif /* CONFIG_CTS_EARPHONE_DETECT */
...
{
#if 0//def CONFIG_CTS_ESD_PROTECTION
    struct chipone_ts_data *cts_data =
        container_of(cts_dev, struct chipone_ts_data, cts_dev);
#endif /* CONFIG_CTS_ESD_PROTECTION */
    int ret;
...
#ifdef CONFIG_CTS_CHARGER_DETECT
int cts_charger_plugin2(void)
{
    int ret;

    cts_info("Charger plugin");
    ret = cts_send_command(cts_dev2, CTS_CMD_CHARGER_PLUG_IN);
    if (ret) {
        cts_err("Send CMD_CHARGER_PLUG_IN failed %d", ret);
    } else {
        cts_dev2->rtdata.charger_exist = true;
    }
    return 0;
}

int cts_charger_plugout2(void)
{
    int ret;

    cts_info("Charger plugout");
    ret = cts_send_command(cts_dev2, CTS_CMD_CHARGER_PLUG_OUT);
    if (ret) {
        cts_err("Send CMD_CHARGER_PLUG_OUT failed %d", ret);
    } else {
        cts_dev2->rtdata.charger_exist = false;
    }
    return 0;
}
...
int cts_earphone_plugin2(void)
{
    int ret;

    cts_info("Eearphone plugin");
    ret = cts_send_command(cts_dev2, CTS_CMD_EP_PLUG_IN);
    if (ret) {
        cts_err("Send Eearphone CTS_CMD_EP_PLUG_IN failed %d", ret);
    } else {
        cts_dev2->rtdata.earphone_exist = true;
    }
    return 0;
}

int cts_earphone_plugout2(void)
{
    int ret;

    cts_info("Earphone plugout");
    ret = cts_send_command(cts_dev2, CTS_CMD_EP_PLUG_OUT);
    if (ret) {
        cts_err("Send Eearphone CTS_CMD_EP_PLUG_OUT failed %d", ret);
    } else {
        cts_dev2->rtdata.earphone_exist = false;
    }
    return 0;
}

bool cts_is_earphone_exist(struct cts_device *cts_dev)
{
    return cts_dev->rtdata.earphone_exist;
}
...
int cts_earphone_plugin(struct cts_device *cts_dev)
{
    ...
    cts_info("Eearphone plugin");
    ret = cts_send_command(cts_dev, CTS_CMD_EP_PLUG_IN);
    if (ret) {
        cts_err("Send Eearphone CTS_CMD_EP_PLUG_IN failed %d", ret);
    } else {
        cts_dev->rtdata.earphone_exist = true;
    }
    return 0;
}
...
int cts_earphone_plugout(struct cts_device *cts_dev)
{
    ...
    cts_info("Earphone plugout");
    ret = cts_send_command(cts_dev, CTS_CMD_EP_PLUG_OUT);
    if (ret) {
        cts_err("Send Eearphone CTS_CMD_EP_PLUG_OUT failed %d", ret);
    } else {
        cts_dev->rtdata.earphone_exist = false;
    }
    return 0;
}

...
#if defined(TOUCHPANEL_GESTURE)
#include "cts_builtin_firmware_yk676_trx_s606_hdplus.h"
#elif defined(YK676_CUSTOMER_CAIFU_BSQ06_HDPLUS) //dtw 20191121
#include "cts_builtin_firmware_yk676_bsq09.h"
#else
...
u32 crc32_chipone(const u8 *data, size_t len)
{
        ...
        const struct cts_firmware *firmware)
{
    ...
    crc = crc32_chipone(FIRMWARE_SECTION(firmware), FIRMWARE_SECTION_SIZE(firmware));
    ...
    if (DDIPARAM_SECTION_ENABLE(firmware) == CTS_SECTION_ENABLE_FLAG) {
        crc = crc32_chipone(DDIPARAM_SECTION(firmware), DDIPARAM_SECTION_SIZE(firmware));
        ...
        struct cts_firmware_sect_info *info)
{
    ...
    info->firmware_sect_crc = crc32_chipone(firmware->data, firmware->size);

...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_EXIT();
	//add by ybx 
	//fts_data->ic_info.ids.chip_idh
	fts_read_reg(0xa6, &tp_version);
	sprintf(mtk_tp_version,"%02x",tp_version);
	//sprintf(mtk_tp_version,"%s",FTS_DRIVER_VERSION);
	sprintf(mtk_tp_info,"%s",FTS_DRIVER_NAME);
	//add end
    return 0;

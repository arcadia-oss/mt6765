...
#undef FPT_PDAF_SUPPORT   //for pdaf switch
#undef TPLINK_S5K3L6_NO_OTP
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	//.checksum_value = 0x49c09f86,  0x6ed0b036
	.checksum_value = 0x87b07dea,//0x9d1d37b8,//0x9d1d37b8 0422  0xe249ffca is smt crc cjc s607
...
//min shutter
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr, //SENSOR_OUTPUT_FORMAT_RAW_Gr //xjl 20181124
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =
 //Glenn added for 3L6 PDAF,20190131 
{
    .i4OffsetX = 28,
...
//PIP 15fps settings,?\A1\A7\A1\E8?\A8\A4\A1\A7\A1\A7Full 30fps
//$MV1[MCLK:24,Width:4208,Height:3120,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1200,pvi_pclk_inverse:0]
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id == imgsensor_info.sensor_id) {
		#ifdef TPLINK_S5K3L6_NO_OTP
				pr_info("Read ok sensor id: 0x%x, img sensor_id:0x%x.\n",
					*sensor_id, imgsensor_info.sensor_id);
				return ERROR_NONE;
		#else
			if(read_otp_module_id()!=0x42)//0x42helitai 0x05daling
			{
				pr_info("Read ok sensor id: 0x%x, img sensor_id:0x%x.\n",
					*sensor_id, imgsensor_info.sensor_id);
				return ERROR_NONE;
...
static kal_uint32 open(void)
{
	...
	if (sensor_id == imgsensor_info.sensor_id) {
			...
			//pr_info("i2c read_otp_module_id 0x%x\n",read_otp_module_id());	
			#ifdef TPLINK_S5K3L6_NO_OTP
...
/* The frame of setting shutter default 0 for TG int */
	...
	#ifdef FPT_PDAF_SUPPORT
	sensor_info->PDAF_Support = 1;//PDAF_SUPPORT_RAW_LEGACY;
	#endif
	switch (scenario_id) {

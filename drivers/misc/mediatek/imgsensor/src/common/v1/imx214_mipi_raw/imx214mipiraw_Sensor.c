...
//#if defined(IMX214_SET_HV_MIRROR)
...
static void set_dummy(void)
{
   //prize fengshangdong modify at 20190112
	printk("imx214 dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	printk("imx214 framerate = %d, min framelength should enable = %d\n", framerate,min_framelength_en);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	printk("imx214 shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);

	//printk("imx214 frame_length = %d ", frame_length);
    //prize end 
}	/*	write_shutter  */
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if (gain < BASEGAIN || gain > 16 * BASEGAIN) {
        printk("imx214 Error gain setting");
    ...
    spin_unlock(&imgsensor_drv_lock);
    printk("imx214 gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
static void ihdr_write_shutter_gain(kal_uint16 le, kal_uint16 se, kal_uint16 gain)
{
    ...
    kal_uint16 reg_gain;
	printk("imx214 le:0x%x, se:0x%x, gain:0x%x\n",le,se,gain);
    spin_lock(&imgsensor_drv_lock);
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x0344,0x00);//x_add_sta 24
    write_cmos_sensor(0x0345,0x18);//x_add_sta
    write_cmos_sensor(0x0346,0x00);
    write_cmos_sensor(0x0347,0x00);
    write_cmos_sensor(0x0348,0x10);//x_add_end 4183
    write_cmos_sensor(0x0349,0x57);//x_add_end
    ...
    write_cmos_sensor(0x034C,0x08);//x_out_size 2080
    write_cmos_sensor(0x034D,0x20);//x_out_size
    ...
    write_cmos_sensor(0x040C,0x08);//digital_crop_image_width 2080
    write_cmos_sensor(0x040D,0x20);//digital_crop_image_width
...
{
    printk("imx214 preview_setting_mHDR\n");
    write_cmos_sensor(0x0100,0x00);
    write_cmos_sensor(0x0114,0x03);
    ...
    */
    ...
    write_cmos_sensor(0x0138,0x01);
    write_cmos_sensor(0x0100,0x01);
    printk("imx214 preview_setting_mHDR mode 0x0220(0x21), Ratio 0x0222(0x08)\n");

}	/*	preview_setting  */

static void capture_setting(kal_uint16 currefps)
{
	printk("imx214 E! currefps:%d\n",currefps);
    // full size 29.76fps
    ...
    if(currefps==300)
    {
        ...
        write_cmos_sensor(0x0341,0x9C);//frame_length_lines
        write_cmos_sensor(0x0342,0x13);
        write_cmos_sensor(0x0343,0x90);
        write_cmos_sensor(0x0344,0x00);
        write_cmos_sensor(0x0345,0x18);//x_addr_start
        write_cmos_sensor(0x0346,0x00);
        write_cmos_sensor(0x0347,0x00);
        write_cmos_sensor(0x0348,0x10);
        write_cmos_sensor(0x0349,0x57);//x_addr_end
        write_cmos_sensor(0x034A,0x0C);
        write_cmos_sensor(0x034B,0x2F);
        write_cmos_sensor(0x0381,0x01);
        write_cmos_sensor(0x0383,0x01);
        ...
        write_cmos_sensor(0x034D,0x40);//x_output_size
        ...
        write_cmos_sensor(0x040D,0x40);//digital_crop_image_width
        ...
        write_cmos_sensor(0x0203,0x8A);//coarse_integ_time
        write_cmos_sensor(0x0224,0x01);
        write_cmos_sensor(0x0225,0xF4);
...
static kal_uint32 open(void)
{
	...
	printk("imx214 PLATFORM:MT6595,MIPI 2LANE\n");
	printk("imx214 preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		default:
			printk("imx214 Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	printk("imx214 enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	printk("imx214 scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
        		    ...
        		    if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
                    printk("imx214 Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			set_dummy();
			printk("imx214 error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}
	return ERROR_NONE;
}
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
            printk("imx214 feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n", imgsensor.pclk,imgsensor.current_fps);
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
            printk("imx214 current fps :%d\n", (UINT32)*feature_data);
			spin_lock(&imgsensor_drv_lock);
            imgsensor.current_fps = *feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
            printk("imx214 ihdr enable :%d\n", (BOOL)*feature_data);
			spin_lock(&imgsensor_drv_lock);
			imgsensor.ihdr_mode = *feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_GET_CROP_INFO:
            printk("imx214 SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
		...
		case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            printk("imx214 SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
            ihdr_write_shutter_gain((UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
			break;
        case SENSOR_FEATURE_GET_VC_INFO:
            printk("imx214 SENSOR_FEATURE_GET_VC_INFO %d\n", (UINT16)*feature_data);
        ...
        case SENSOR_FEATURE_SET_HDR_SHUTTER:
            printk("imx214 SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1));

...
static int lsm6ds3_acc_suspend(struct device *dev); //struct i2c_client *client, pm_message_t msg);
static int lsm6ds3_acc_resume(struct device *dev); //struct i2c_client *client);
#endif
...
static int calib_offset[3] = {0,0,0};                  // ug unit \A1\EA? used for st hal calib
#endif
/*st-sw caoyi1 add for Gsensor APK calibration 20150512 end*/
...
//static u8 *gpDMABuf_va = NULL; //xjl 20160823
...
static int LSM6DS3_ReadAccData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	#if 1
//yx 20160831
              if(abs(obj->data[LSM6DS3_AXIS_X]) >= 1000)
                     obj->data[LSM6DS3_AXIS_X] = ((long)(obj->data[LSM6DS3_AXIS_X]) * obj->sensitivity/1000)*GRAVITY_EARTH_1000/1000;
              else
              obj->data[LSM6DS3_AXIS_X] = (long)(obj->data[LSM6DS3_AXIS_X]) * obj->sensitivity*GRAVITY_EARTH_1000/(1000*1000); //NTC

              if(abs(obj->data[LSM6DS3_AXIS_Y]) >= 1000)
                     obj->data[LSM6DS3_AXIS_Y] = ((long)(obj->data[LSM6DS3_AXIS_Y]) * obj->sensitivity/1000)*GRAVITY_EARTH_1000/1000;
              else
              obj->data[LSM6DS3_AXIS_Y] = (long)(obj->data[LSM6DS3_AXIS_Y]) * obj->sensitivity*GRAVITY_EARTH_1000/(1000*1000);

              if(abs(obj->data[LSM6DS3_AXIS_Z]) >= 1000)
                     obj->data[LSM6DS3_AXIS_Z] = ((long)(obj->data[LSM6DS3_AXIS_Z]) * obj->sensitivity/1000)*GRAVITY_EARTH_1000/1000;
              else
...
/* do reset to ensure hardware inital state -- add by liaoxl.st 8.27.2005 end */
	...
	printk("LSM6DS3_acc_CheckDeviceID ret=%d\n", res);
	if(res != LSM6DS3_SUCCESS)
	{
		return res;
	}
	...
	printk("LSM6DS3_acc_Set_RegInc ret=%d\n", res);
	if(res != LSM6DS3_SUCCESS)
	{
		return res;
	}
	...
	printk("LSM6DS3_acc_SetFullScale ret=%d\n", res);
	if(res != LSM6DS3_SUCCESS) 
	{
		return res;
	}
	...
	res = LSM6DS3_acc_SetSampleRate(client, obj->sample_rate);
	printk("LSM6DS3_acc_SetSampleRate ret=%d\n", res);
	if(res != LSM6DS3_SUCCESS ) 
	{
		return res;
	}
	...
	printk("LSM6DS3_acc_SetPowerMode ret=%d\n", res);
	if(res != LSM6DS3_SUCCESS)
	{
		return res;
	}
	...
	printk("LSM6DS3_acc_init_client OK!\n");
	//acc setting
...
#endif
	err = LSM6DS3_init_client(lsm6ds3_i2c_client, 0);
	if (err) {
...
#endif
	if (sensor_power == false) {
		err = LSM6DS3_acc_SetPowerMode(lsm6ds3_i2c_client, true);
		if (err)
...
static int lsm6ds3_factory_get_raw_data(int32_t data[3])
{
	...
	LSM6DS3_ReadAccRawData(lsm6ds3_i2c_client, (s16*)strbuf);
...
static int lsm6ds3_factory_clear_cali(void)
{
	...
	err = LSM6DS3_acc_ResetCalibration(lsm6ds3_i2c_client);
	if (err) {
		GSE_ERR("LSM6DS3_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int lsm6ds3_factory_set_cali(int32_t data[3])
{
	...
	err = LSM6DS3_acc_WriteCalibration(lsm6ds3_i2c_client, cali);
	if (err) {
		GSE_ERR("LSM6DS3_WriteCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int lsm6ds3_factory_get_cali(int32_t data[3])
{
	...
	err = LSM6DS3_acc_ReadCalibration(lsm6ds3_i2c_client, cali);
	if (err) {
		GSE_ERR("LSM6DS3_ReadCalibration failed!\n");
		return -1;
	}
...
//#if defined(Q858_CONFIG)||defined(Q811_CONFIG)||defined(YK660_CONFIG)||defined(YK678_CONFIG)   //yx 20160824
//#if defined(CONFIG_MTK_FAN5405_SUPPORT) // xen 20161220
//	client->addr = 0x6B;//0x6A;
//#else
//	client->addr = 0x6A;
//#endif
//#else
...
/* init step counter intterupt vars  --add by liaoxl.st 7.28.2015  end */
	...
	/* for DMA transfer */
//	gpDMABuf_va = (u8 *) dma_alloc_coherent(&client->dev, IIC_DMA_MAX_TRANSFER_SIZE, (dma_addr_t *)&gpDMABuf_pa, GFP_KERNEL); //xjl 20160823
//	if (!gpDMABuf_va) {
...
//	memset(gpDMABuf_va, 0, IIC_DMA_MAX_TRANSFER_SIZE);
	...
	printk("lsm6ds3_acc_probe LSM6DS3_init_client ret=%d\n", err);
	if(err)
	{
...
#if defined(ANDROID_O_PLATFORM) //xen 20180208
	err = accel_factory_device_register(&lsm6ds3_acc_factory_device);
#else
...
static int lsm6ds3_local_init(void)
{
	...
	printk("lsm6ds3_acc_local_init\n");
	//KXTJ2_1009_power(hw, 1);
	if (i2c_add_driver(&lsm6ds3_i2c_driver)) {
		GSE_ERR("add driver error\n");
		return -1;
	}
	printk("lsm6ds3_acc_local_init lsm6ds3_acc_init_flag=%d\n", lsm6ds3_acc_init_flag);

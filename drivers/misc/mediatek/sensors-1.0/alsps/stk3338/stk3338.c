...
// light start
#define PROC_LSENSOR_INFO "driver/lsensor_info"
#define PROC_PSENSOR_INFO "driver/psensor_info"
// light end

//#define USE_OLD_SENSOR_DTS_ARCH
...
/*----------------------------------------------------------------------------*/
#if 1//defined(YK687_CUSTOMER_CAIFU_WS200_FHDPLUS)||defined(YK685_CUSTOMER_CAIFU2_BSM01_HDPLUS)||defined(YK676_CUSTOMER_KEMI_LT600_HDPLUS)||defined(YK676_V60_CUSTOMER_TRX_S607_HDPLUS)||defined(YK185_CONFIG)
...
#else
static struct attribute *stk3x1x_attr_list[] = {
    &dev_attr_als.attr,
    &dev_attr_ps.attr,    
    &dev_attr_ir.attr,    
...
static struct attribute_group stk3x1x_attribute_group = {
	.attrs = stk3x1x_attr_list
};
...
#else
	int err = 0;
	err = sysfs_create_group(&stk3x1x_obj->client->dev.kobj, &stk3x1x_attribute_group);
	if (err < 0) {
		pr_err("unable to create STK3x1x  attribute file\n");
		return -3;
	} 
...
static int stk3x1x_delete_attr(struct device_driver *driver)
{
	...
	int err = sysfs_remove_group(&stk3x1x_obj->client->dev.kobj,
			   &stk3x1x_attribute_group);	
	return err;
#endif
}
...
// light start
static int proc_lsensor_info_read(struct seq_file *m, void *v) {
        seq_printf(m, "%s\n", stk3x1x_DEV_NAME);
        return 0;
}

static int proc_lsensor_info_open(struct inode *inode, struct file *file) {
        return single_open(file, proc_lsensor_info_read, NULL);
}

static struct file_operations proc_lsensor_fops = {
        .owner = THIS_MODULE,
        .open  = proc_lsensor_info_open,
        .read  = seq_read,
};

static int proc_psensor_info_read(struct seq_file *m, void *v) {
        seq_printf(m, "%s\n", stk3x1x_DEV_NAME);
        return 0;
}

static int proc_psensor_info_open(struct inode *inode, struct file *file) {
        return single_open(file, proc_psensor_info_read, NULL);
}

static struct file_operations proc_psensor_fops = {
        .owner = THIS_MODULE,
        .open  = proc_psensor_info_open,
        .read  = seq_read,
};
// light end
...
//	ps_ctl.is_report_input_direct = true;
	...
	ps_lock = wakeup_source_register(&client->dev, "ps wakelock");
...
#endif
// light start
        proc_create(PROC_LSENSOR_INFO, 0, NULL, &proc_lsensor_fops);
        proc_create(PROC_PSENSOR_INFO, 0, NULL, &proc_psensor_fops);
// light end
...
#endif		
// light start
        remove_proc_entry(PROC_LSENSOR_INFO, NULL);
        remove_proc_entry(PROC_PSENSOR_INFO, NULL);
// light end
#ifdef STK_GES		
	input_unregister_device(obj->ges_input_dev);	
	input_free_device(obj->ges_input_dev);		

	...
	if (scene_param->cfg_update_flag == AW87XXX_CFG_OK) {
		...
		for (i = 0; i < load_cnt->len; i = i + 2) {
			...
			if (aw87xxx->gain_hold_flag
			    && (load_cnt->data[i] == aw87xxx->gain_reg_addr))
				continue;
...
static int aw87xxx_actflag_thread(void *pdata)
{
	...
	while (!kthread_should_stop()) {
		if (aw87xxx->current_mode == AW87XXX_OFF_MODE
			...
			aw_dev_info(aw87xxx->dev, "%s, get hal msg: left_flag: %d, right_flag: %d\n",
					  __func__, active_flag[0], active_flag[1]);

			ret = aw87xxx_i2c_read(aw87xxx, aw87xxx->gain_reg_addr, &reg_val);
			if (ret < 0) {
				pr_err
				  ...
				  const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
		mutex_lock(&aw87xxx->lock);
		aw87xxx->active_flag_val[0] = databuf[0];
		aw87xxx->active_flag_val[1] = databuf[1];
		aw_dev_info(aw87xxx->dev,
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	flag[0] = aw87xxx->active_flag_val[0];
	flag[1] = aw87xxx->active_flag_val[1];
				       ...
				       const char *buf, size_t len)
{
	...
	ret = kstrtou8(buf, 0, &val);
	if (ret) {
		aw_dev_err(aw87xxx->dev,"%s: invalid input to auto_thread\n", __func__);
		return ret;
	}
	mutex_lock(&aw87xxx->lock);
	if (!val) {
...
static DEVICE_ATTR(actflag, S_IWUSR | S_IRUGO, aw87xxx_get_actflag, aw87xxx_set_actflag);
...
* aw87xxx parse dts
*****************************************************************************/
static void aw87xxx_parse_gpio_dt(struct aw87xxx *aw87xxx, struct device_node *np)
{
	aw_dev_info(aw87xxx->dev, "%s enter, dev_i2c%d@0x%02X\n", __func__,
			aw87xxx->i2c_seq, aw87xxx->i2c_addr);
